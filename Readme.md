 ...
 content.getFeed:               // Получение ленты постов
     ...
     params:                    // Параметры запроса из гейта
         type <string>          // Тип ленты
             [
               natural          // Натуральная лента, в порядке появления постов 
             | personal         // Персональная лента, подписки пользователя
             | promo            // Промо лента, рекламная проплаченных постов
             | popular          // Популярная лента, на основе алгоритма ранжирования
             | actual           // Актуальная лента, на основе алгоритма ранжирования
             ]          
         tags <string[]>([])    // Теги для фильтрации, опционально
         fromId <string>(null)  // ID с которого нужно начать показывать историю, опционально
